...
| `support_space_opened` | Sharing space opened from existing `طمنّا عليك` page | Client check-in / support space | Local debug event only | LOW | SESSION | Client/premium | NONE | LOCAL_DEBUGPRINT_ONLY | No backend signal; no personal data. |
| `support_space_write_clicked` | Write mode selected | Client support space | Local debug event only | LOW | SESSION | Client/premium | NONE | LOCAL_DEBUGPRINT_ONLY | Internal mode switch only. |
| `support_space_message_submitted` | Short beta message submitted | Client support space | Local debug event only | MEDIUM | SESSION_LOCAL_ONLY | Client/premium | NONE | LOCAL_DEBUGPRINT_ONLY | Message stored only in memory; debug includes length/status only. |
| `support_space_message_blocked` | Message failed validation or denylist | Client support space | Local debug event only | LOW | SESSION | Client/premium | NONE | LOCAL_DEBUGPRINT_ONLY | Debug includes reason category only. |
| `support_space_messages_viewed` | Recent messages panel viewed | Client support space | Local debug event only | LOW | SESSION | Client/premium | NONE | LOCAL_DEBUGPRINT_ONLY | No backend emission. |
| `support_space_survey_opened` | Future survey opened | Client support space | `G2SupportSpaceSurveyPayload` planned | LOW | SESSION | Client/premium | NONE | DOCUMENTED_ONLY | Survey not wired in current placement update. |
| `support_space_survey_submitted` | Future survey submitted | Client support space | `G2SupportSpaceSurveyPayload` planned | MEDIUM | TEMPORARY | Client/premium | NONE | DOCUMENTED_ONLY | Survey output should remain debugPrint only when wired. |
